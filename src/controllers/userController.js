...
const updateContent = async(req, res) => {
    try{
        ...
        const updatedContent = await UserServices.updateContent(contentId,newContent);
...
const updateContentField = async(req, res)=>{
    try{
        ...
        const contentType = await UserServices.updateContentField(contentId,changeField);
...
const addContentEntries = async(req, res)=>{
    try{
        ...
        const contentTypeEntries = await UserServices.addContentEntries(contentId,newEntry);
        res.status(200).json(contentTypeEntries);
    }
    catch(error){
        res.status(500).json(error);
...
const updateContentEntries = async(req, res)=>{
    try{
        const {contentId} = req.params;
        const {newEntry} = req.body;
        const contentTypeEntries = await UserServices.updateContentEntries(contentId,newEntry);
        res.status(200).json(contentTypeEntries);
    }
    catch(error){
        res.status(500).json(error);
...
module.exports = {addContent,getContents,addFields,updateContent,updateContentField,deleteContentField,getFieldCount,getContentEntries,addContentEntries,updateContentEntries};